#CCC #Kern 
[[Testimonials]]


### 🧠 **Berufliches Profil**

#### 1. **Tätigkeit**

Renommierter Seminartrainer, Coach, (studierter) Mediator, Vortragsredner (Speaker), Autor (ein bisschen)

##### **Kernthema (=thematische Klammer aller Dienstleistungen) seit über 25 Jahren: Ich stärke Kooperation in Unternehmen.

* **das ist gut für die FIRMA (mehr Profit) und gut für die Menschen (mehr Wohlergehen;** gemäß längster diesbzgl. jemals durchgeführter Langzeitstudie der Harvard Universität ("Harvard Study of Adult Development") leben Menschen mit guten menschlichen Beziehungen 8 Jahre länger (und besser)*
* Für die Firmen: Weniger Reibung, mehr Produktivität
- Stärkung der **Beziehungsebene** und der "Chemie-Kompetenz" (#ChemieKompetenz) als Schlüssel zu Produktivität und Führungserfolg
   
#### 2. **Headline im Linkedin-Profil

🔴 Der "#MiteinanderMagier". Ich stärke Kooperation: Menschen werden kompatibler, Teams wirksamer, Firmen erfolgreicher. Durch das "8 grüne Ampeln"-System, Seminare (ausgezeichnet von Stiftung Warentest) und Mediation.


#### 2. **Besonderheit, USP: Eigene Methode

#### Das "8 grüne Ampeln"-System (8gA). 
Von Werner Schienle entwickeltes einzigartiges System, Trainingsformat.

Kurz gesagt: Ein hochwirksames System zur Verbesserung zwischenmenschlicher Kompatibilität. 
In Zusammenarbeit und Führung.
    
Genaueres siehe Notiz: [[8 grüne Ampeln-System]]  (Wichtig!)

#### 2. **Vertrauensanker**

* schon vor vielen Jahren **Testsieger Stiftung Warentest** (mit Konfliktmanagement-Seminaren; das betraf also nicht das 8 grüne Ampeln-System)
* Nr. 1 Bestseller-Autor im renommierten Fachverlag Springer. Titel "Psychologisches Konfliktmanagement: Professionelles Handwerkszeug für Fach- und Führungskräfte" (2025 auch auf Englisch erschienen)
* Das ARD-Fernsehen nannte ihn "Red Adair", in Anlehnung an den berühmten Feuerwehrmann, der auch aussichtslos scheinende Feuer oft löschen konnte
* Zahlreiche hervorragende Testimonials von allen Hierarchie-Ebenen (bis hoch auf Vorstandsebene Dax-Unternehmen), darunter auch viele Video-Testimonials. Und hervorragende Empfehlungen in der Empfehlungs-Section im Linkedin-Profil
* TV- und Presseauftritte (allerdings in den letzten paar Jahren nicht mehr)

#### 3. **Marke & Positionierung**

* Der **"#MiteinanderMagier**"
  ich nenne es einen "sprachlichen Henkel". Ich glaube in der Marketingsprache heißt es:  Sein Brand-Hook (verbaler Anker, linguistischer Anker, phrase Hook)
* Diesen Brand-Hook nutze ich erst seit Q2/2025. Soll künftig zwecks Markenstärkung und Wiedererkennung relativ konsequent genutzt werden
* Der Begriff MiteinanderMagier Hängt auch damit zusammen, dass Kunden nach einer Mediation über das Ergebnis mitunter von "Wunder" sprechen.
* Ein Unternehmer (Jörg Rauschenberger, ca. 400 Mitarbeiter) sagte über ihn in einem Video-Testimonial: "Es ist beeindruckend, welche durchschlagenden Ergebnisse die Werner Schienle-Magie selbst in scheinbar ausweglosen Situationen liefert!"
 
* Ich nutze den Hashtag "#DenkAnstosser" in Linkedin Postings und in Newslettern*     
* Mein Stil (laut Analyse von ChatGPT): **präzise, klar, tiefgründig**, mit Anspruch an inhaltliche Substanz


#### **Social Media**

* Linkedin (intensiv)*
* Weitere systematische Kunden-Outreach mit eigenem Newsletter (ganz grobe Zahl: 20x im Jahr)
* Youtube
* Twitter (X), aber fast gar nicht gepflegt


#### 3. **ICP (Ideal Customer Profile)**

Inhalt siehe Notiz: [[ICP-Definition]]
- 

#### 4. **Angebotsformate** (fast nur B2B)


- Seminare, insbesondere zum Thema Konfliktmanagement / Konfliktkompetenz (da wurde ich Testsieger Stiftung Warentest). Überwiegend in Präsenz
- Mediation (in Präsenz sowie online)
- Führungskräfte-Coaching (in Präsenz sowie online)
- Coaching-"Flatrates" (z. B. 3, 6 oder 12 Monate) (überwiegend online)
* Vorbereitung auf Mediationen mit 8gA (online)
* Trainings spezieller Teilkompetenzen 
	- z. B. „Stoppschild-Technik“ (online)
* (ist vermutlich noch nicht vollständig)*
 

